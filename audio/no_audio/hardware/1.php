...
if (isset($_POST['submitting'])) {
    ...
    if (($device === "laptop") || ($device === "desktop") || ($device === "tablet") || ($device === "phone") || ($device === "laptop2tv") || ($device === "cast2tv") || ($device === "tv") || ($device === "other")) {
        ...
        if ($device === "laptop") {
            header("Location: laptop/1.php");
        }else {
            if ($device === "desktop") {
                header("Location: desktop/1.php");
            }else {
                if ($device === "tablet") {
                    header("Location: phone/1.php");
                }else {
                    if ($device === "phone") {
                        header("Location: phone/1.php");
                    }else {
                        if ($device === "laptop2tv") {
                            header("Location: laptop2tv/1.php");
                        }else {
                            if ($device === "cast2tv") {
                                header("Location: casting/1.php");
                            }else {
                                if ($device === "tv") {
                                    header("Location: tv/1.php");
                                }else {
