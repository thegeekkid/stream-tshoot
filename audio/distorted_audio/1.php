...
if (isset($_POST['submitting'])) {
    ...
    if ($hear === "yes") {
        $message = "ID: $id \n
        The test audio was also distorted.";
        send_message($slack_hook, $message);
        header("Location: 2.php");
    }else {
        if ($hear === "no") {
            $message = "ID: $id \n
        The test audio was *not* distorted.";
            send_message($slack_hook, $message);
...
<p>Please play a few seconds of audio below and select whether or not that is also distorted.</p>
<audio controls>
...
<form id="issue_type" action="1.php" method="post">
    ...
    <label for="hear">Was the audio above distorted?</label>
